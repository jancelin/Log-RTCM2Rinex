...
# Thin wrapper for the hourly (1s) RINEX converter.
# No code duplication: the whole logic lives in rinex-converter.sh.

export CONVERTER_NAME="${CONVERTER_NAME:-converter1s}"

# Per-role overrides (do NOT depend on /config/.env values)
export FORCE_RINEX_HOURLY_ENABLE="${FORCE_RINEX_HOURLY_ENABLE:-true}"
export FORCE_RINEX_DAILY_ENABLE="${FORCE_RINEX_DAILY_ENABLE:-false}"
export FORCE_CLEANUP_ENABLE="${FORCE_CLEANUP_ENABLE:-false}"

# Tmp sur /dev/shm (tmpfs RAM) pour éliminer la contention I/O disque/NFS.
# Taille shm_size allouée dans docker-compose.yml (2g pour converter1s).
